...
from app.agents.core.models import ToolContext, ToolResult
...
class ToolExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            tool_result = ToolResult(ok=True, status="success", data=_serialize(result))
            ...
            tool_result = ToolResult(
                ok=False, status="error", error=exc.message, error_code=exc.code
            )
        except Exception:
            logger.exception(
